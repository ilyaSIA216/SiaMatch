...
function hideAnimatedWelcomeScreen() {
  ...
  // 0-2 сек: Клевер появляется
  // 2-3.5 сек: Заголовок "SiaMatch" появляется
  // 3.5 сек: Текст "Любовь начинается здесь" появляется ВНУТРИ сердца
  // 3.6-6.6 сек: Вокруг текста рисуется сердце (3 секунды)
  // 6.6-7.5 сек: Пауза
  // Итого: 7.5 секунд
  ...
  }, 7500); // Общее время показа 7.5 секунд
}
